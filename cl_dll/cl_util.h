...
inline int SPR_Height(HSPRITE_ x, int f) { return gEngfuncs.pfnSPR_Height(x, f); }
inline int SPR_Width(HSPRITE_ x, int f) { return gEngfuncs.pfnSPR_Width(x, f); }
...
HSPRITE_ LoadSprite(const char* pszName);
